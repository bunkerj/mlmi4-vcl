...
def coreset_rand(x_coreset, y_coreset, x_train, y_train, coreset_size):
    ...
    # at first timestep, x_coreset should be set to "torch.FloatTensor()"
    # and y_coreset should be set to "torch.ByteTensor()"
...
# K-center Selection
def coreset_k(x_coreset, y_coreset, x_train, y_train, coreset_size):
    ...
    x_coreset = torch.cat([x_coreset, x_train[idx,:]], dim=0)
    y_coreset = torch.cat([y_coreset, y_train[idx]], dim=0)
